...
// ... (Mantém suas funções acquireLock e releaseLock no topo) ...

// --- COLE O NOVO BLOCO AQUI ---
let config = {
  apiUrl: "http://localhost:3000",
  jwt: "ACESSO_LIVRE_ATIVADO",
  contaAtiva: "ADMIN_PRINCIPAL"
};

chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.set({
    sk_api_url: config.apiUrl,
    sk_jwt: config.jwt,
    sk_conta_ativa: config.contaAtiva
  });
});
...
// ... (O resto do seu código original segue abaixo) ...
...
chrome.storage.local.get(["sk_api_url", "sk_jwt", "sk_conta_ativa"], (res) => {
  // ... (todo esse bloco de leitura inicial)
});
